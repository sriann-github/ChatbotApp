...
# Define the 6 incident templates and placeholder pools
incident_templates = [
    "User reported {subcategory} issue causing {random_effect}, impacting user satisfaction and business operations.",
    "System encountered {random_issue} in {subcategory}, resulting in {random_effect}.",
    "Service disruption in {subcategory} due to {random_cause}, leading to {random_effect}.",
    "Performance degradation observed in {subcategory} with {random_metric}, requiring immediate attention.",
    "Error reported in {subcategory}: {random_issue}. Impact on {random_entity} observed.",
    "Operational downtime in {subcategory} traced to {random_cause}, causing {random_effect}."
]

placeholder_pools = {
    "random_issue": ["high latency", "frequent errors", "unexpected outages", "data corruption", "failed API calls"],
    "random_cause": ["network misconfiguration", "service downtime", "resource overload", "unauthorized access"],
    "random_effect": ["business disruption", "revenue loss", "client dissatisfaction", "service inconsistency"],
    "random_metric": ["99.9% failure rate", "high CPU usage", "memory leaks detected"],
    "random_entity": ["end-user experience", "critical operations", "service reliability"]
}
...
# Function to generate a single incident description dynamically
def generate_dynamic_incident(subcategory):
    try:
        return random.choice(incident_templates).format(
            subcategory=subcategory,
            random_issue=random.choice(placeholder_pools["random_issue"]),
            random_cause=random.choice(placeholder_pools["random_cause"]),
            random_effect=random.choice(placeholder_pools["random_effect"]),
            random_metric=random.choice(placeholder_pools["random_metric"]),
            random_entity=random.choice(placeholder_pools["random_entity"])
        )
    except KeyError as e:
        print(f"KeyError: {e}. Check placeholder pools or templates.")
        raise    

# Function to dynamically generate unique incidents in a chunk
def generate_incident_chunk(chunk_size, users_df, categories_df):
    incident_list = []
    unique_descriptions = set()
    start_date = datetime.datetime(2024, 10, 1)
    end_date = datetime.datetime(2024, 10, 30)

    while len(unique_descriptions) < chunk_size:
        for _, category in categories_df.iterrows():
            if len(unique_descriptions) >= chunk_size:
                break

            # Generate a unique description
            incident_desc = generate_dynamic_incident(category['subcategory_name'])
            ...
            root_cause = generate_dynamic_incident(category['subcategory_name'])
            resolution = generate_dynamic_incident(category['subcategory_name'])
            solution = generate_dynamic_incident(category['subcategory_name'])
            severity = random.choice(["Low", "Medium", "High"])

            # Select business impact based on severity
            business_impact_templates = {
                "Low": f"Minimal impact caused by {random.choice(placeholder_pools['random_cause'])} in {category['subcategory_name']}.",
                "Medium": f"Moderate disruption observed in {category['subcategory_name']} due to {random.choice(placeholder_pools['random_cause'])}.",
                "High": f"Critical issue in {category['subcategory_name']} causing {random.choice(placeholder_pools['random_effect'])}."
            }
            business_impact = business_impact_templates[severity]
            ...
            incident = {
                ...
                'updated_at': random_date(start_date, end_date) if random_status() != 'Open' else None,
                'severity': severity
            }
...
# Main function to generate incidents with clustered parallel generation
def generate_incidents_parallel(num_incidents, users_df, categories_df, num_processes=4):
    # Determine the size of each chunk
    chunk_size = num_incidents // num_processes

    # Use multiprocessing to generate incidents in parallel
    with Pool(processes=num_processes) as pool:
        # Create arguments for each process
        args = [(chunk_size, users_df, categories_df) for _ in range(num_processes)]

        # Run the processes and collect results
        results = pool.starmap(generate_incident_chunk, args)

    # Combine all results and deduplicate
    combined_incidents = [incident for chunk in results for incident in chunk]
    unique_incidents = {incident['Incident_Description']: incident for incident in combined_incidents}
    
    # Return the final DataFrame with unique incidents
    return pd.DataFrame(list(unique_incidents.values()))
...
def main():
    ...
    # Call the function with the desired number of incidents and processes
    incidents_df = generate_incidents_parallel(3000, users_df, categories_df, num_processes=3)
